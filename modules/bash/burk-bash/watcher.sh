...
function watcher() { 
  ...
  trap 'lines=$(tput lines); watcherheight=$(($lines - 5))' WINCH
  trap 'columns=$(tput cols)' WINCH
  shift

  while true; do
    # print output to tmp file
    ( eval ${CMD_TO_RUN} ) | tail -n ${watcherheight} > ${watcherfile} 2>/dev/null

    clear 
    wrap_print "${watchertime}" "$(date +%H:%M)" "$*"
    cat ${watcherfile} && rm -f ${watcherfile}
